...
HRobot 支持配置文件，配置文件在 mcl/config/com.happysnaker.HRobot/config.json 中，您可以手动创建配置文件，不过建议您运行一次 mcl.cmd，这会自动生产配置文件 config.json，如下是配置文件的模板（最新版插件），**请注意对于 QQ 号或群号请务必用单引号 ‘’ 表示，否则可能会被程序识别为一个整数，我们建议对于任何字符串，都使用单引号以避免歧义**：
