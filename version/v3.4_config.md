...
# v3.4.1 支持(3.4 不支持)，允许 -1, 0, 1, 2 四种配置
# -1 表达不打印任何日志；0 会输出较多日志，用于 debug；1 仅会输出一些重要日志；2 会输出重要日志，并且会记录错误日志到文件中
logLevel: 2
...
# 自动审批配置，当验证消息为空时则会默认同意
# 2022-09-27 声明：QQ 群自带此消息，此功能将在未来废弃
autoApproval:
  - 'QQ 群号': '审批验证消息'
  - 'QQ 群号': ''
...
# v3.4 新增选项 4，4 表示仅图片链接，与 1 的区别在于 1 上传了腾讯服务器，而 4 发送原始链接，手机 QQ 中可能会被视为恶意链接
colorStrategy: 3
# 涩图撤回时间，单位为秒
...
customKeyword:
  '全局关键字': '全局关键字回复 [hrobot::$img](https://tenapi.cn/acg)'  # 使用内部标签 img 返回图片，可参考配置手册
  '#regex#全局正则表达式': '全局正则表达式回复'
  '群号':
    '群内关键字': '群内关键字回复'
    '#regex#群内正则表达式': '群内正则表达式回复'
  # 示例
  '#regex#早.?.?': '[hrobot::$quote](quote)早什么早，早上要说我爱你'    # 匹配以早开头、长度为 1-3 的句子，$quote 是内部标签，可引用触发消息
  ...
  - groupId: '903025723'    # 定时任务发送消息的群
    ...
    content:           # 如果内容为列表，机器人会随机选择发送
      - |-             # 发布的内容，|- 是 yaml 的语法。这允许字符串换行
        [mirai:face:317]  
        [hrobot::$img](https://tenapi.cn/acg)
      - |-             # 发布的内容，|- 是 yaml 的语法。这允许字符串换行
        呃....
        额....
        饿....
        [hrobot::$img](https://tenapi.cn/acg)
        [hrobot::$img](http://h1.ioliu.cn/bing/HallesWood_ZH-CN9790575479_1920x1080.jpg?imageslim)
        [hrobot::$img](/app/config/test.png)
        [hrobot::$at](2199828958)
        上述演示了内部标签 img、at 的用法，可参考配置手册
    image: false
