...
在一些情况下，发送涩图无反应，但查看控制台日志却发送图片已经发送，这种情况可能是由于图片被腾讯 ”拦截“ 了，属于正常现象，如果这种情况频繁发生，你的号就可能被封，这种情况可以暂停使用此号，过会时间再用。
...
**表格展示为最新版 v3.0 插件功能，默认情况下，与机器人聊天需要@机器人，而其他关键字都无须 @，详情请查看下表中的“示例”，你可以在 [历史版本关键字及命令](./HISTORY_VERSION.md) 中找到历史版本对应关键字，在配置文件中你可以选择关闭 @。**
...
下表展示最新版插件 v3.0 插件功能，你可以在 [历史版本关键字及命令](./HISTORY_VERSION.md) 中找到历史版本对应信息。
...
| #设置定时任务       | #设置定时任务{18-0-10-1}你好呀            | 配置定时任务，以{}包裹配置信息，{}中配置形式为 {hour-minute-count-image}，表示在每日的几时几分发送消息，一共发送几次，末尾是否需要附加上一张随机图片，image 配置可省略，默认为 0（否），示例中含义为在每天的 18 点 0 分发送一条内容为 “你好呀” 的消息，并在消息尾部附加上一张随机图片，一共执行 10 次，当 count 为 0 时表示无限执行（直到机器人进程 exit），暂不支持中途取消任务，可以选择停止机器人运行。 | 至少是管理员权限                                     |
