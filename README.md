...
但是你必须要有对应的权限，在最开始，你需要在配置文件中配置你自己成为超级管理员，随后你便可以通过命令的形式授予或移除他人权限，配置文件相关信息可参考：[配置手册](https://github.com/happysnaker/mirai-plugin-HRobot/blob/master/CONFIG.md)。

> 超级管理员 > 管理员 > 群管理员 > 坎公管理员

下表展示最新版插件 v2.0-beta 插件功能，你可以在 [历史版本关键字及命令](./HISTORY_VERSION.md) 中找到历史版本对应信息。
